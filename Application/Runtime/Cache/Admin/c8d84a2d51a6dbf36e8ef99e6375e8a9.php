				...
				<div class="content">
					<div class="col-sm-12">
						<div class="pull-right">
							<a class="btn btn-success" href="<?php echo U('add');?>" ><i class="fa fa-plus-square"></i> 添加活动</a>
						</div>
					</div>
					<table class="no-border blue">
						...
						<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><a href="<?php echo U('edit?id='.$vo['id']);?>"><?php echo ($vo['title']); ?></a></td>
							<td><?php echo get_activity_type($vo['type']);?></td>
							<td><?php echo show_status_op(abs(1-$vo['status']));?></td>
							<td class="text-right">
								...
								<a id="status" data-placement="left" data-toggle="tooltip" data-original-title="状态" class="label <?php if(($vo["status"]) == "0"): ?>label-default<?php else: ?>label-success<?php endif; ?> no-refresh ajax-get" href="<?php echo U('setStatus?Model=activity&id='.$vo['id'].'&status='.abs(1-$vo['status']));?>"><i class="fa <?php if(($vo["status"]) == "0"): ?>fa-eye-slash<?php else: ?>fa-eye<?php endif; ?>"></i></a>
...
<script type="text/javascript" src="/onepay/Public/Admin/js/bootstrap/dist/js/bootstrap.min.js"></script>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			highlight_subnav('<?php echo U('Activity/index');?>');
		});
	</script>
