...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/onepay/Public/Admin/js/bootstrap.datetimepicker/css/bootstrap-datetimepicker.min.css" />
							...
							<div class="pull-right">
								<div class="btn-group">
								  ...
								  <ul class="dropdown-menu" role="menu">
									<li><a href="<?php echo U('order');?>">所有类型</a></li>
				                    <li><a href="<?php echo U('order?recharge=0');?>">购买</a></li>
				                    <li><a href="<?php echo U('order?recharge=1');?>">充值</a></li>
								  </ul>
								</div>
							</div>
						</div>	
						<table class="no-border blue">
							...
							<tbody class="no-border-y">
								<?php if(is_array($order)): $i = 0; $__LIST__ = $order;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php echo ($vo['shop']['name']); ?> 第<?php echo ($vo['shop']['no']); ?>期</td>
									<td><?php echo get_user_name($vo['uid']);?></td>
									<td>￥<?php echo ((isset($vo['number']) && ($vo['number'] !== ""))?($vo['number']):0); ?></td>
									<td><?php echo ($vo['order_id']); ?></td>
									<td><?php echo ($vo['prepay_id']); ?></td>
									<td><?php echo pay_state($vo['type']);?></td>
									<td><?php echo pay_recharge($vo['recharge']);?></td>
...
<script type="text/javascript" src="/onepay/Public/Admin/js/bootstrap/dist/js/bootstrap.min.js"></script>
	...
	<script type="text/javascript" src="/onepay/Public/Admin/js/bootstrap.datetimepicker/js/locales/bootstrap-datetimepicker.zh-CN.js"></script>

	<script type="text/javascript">
		$(document).ready(function(){
			$(".starttime").datetimepicker({autoclose: true,language: 'zh-CN'});
			$(".endtime").datetimepicker({autoclose: true,language: 'zh-CN'});
			highlight_subnav('<?php echo U('Statistics/order');?>');
		})
	</script>
