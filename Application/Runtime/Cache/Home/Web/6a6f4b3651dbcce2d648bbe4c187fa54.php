	...
	<div class="top-backs">
  	...
  	<div class="container" id="shoplist">
  		...
  		<?php if(is_array($shop)): $i = 0; $__LIST__ = $shop;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="ls-product-list">
			...
			<div class="reco-ls huise">总需：<?php echo ($vo["price"]); ?> 人次<?php if(($vo['ten_restrictions']) == "1"): ?><b><i></i>限购<?php echo ($vo['ten_restrictions_num']); ?>人次</b><?php endif; ?></div>
				<div class="progress">
			...
			<div class="reco-bottom">
				<?php if(($vo['ten_restrictions_num']-$vo['no_count'] <= 0) and ($vo['ten_restrictions'] > 0) and (UID != 0)): ?><div id="divEmpty" class="No_shopping">
			            <b></b>
				...
				<div class="ro-goods">
					...
					<div class="ro-goods-inputs">
						...
						<input type="type" value="<?php echo ($vo['ten_unit']); ?>" price="<?php echo ($vo['price']); ?>" unit="<?php echo ($vo['ten_unit']); ?>" surplus="<?php echo ($vo['surplus']); ?>" limit="<?php echo ($vo['ten_restrictions_num']-$vo['no_count']); ?>" name="number">
				...
				<div class="reco-ls-btn"><button class="btn btn-pink" type="submit">立即夺宝</button></div><?php endif; ?>
			</div>
			</form>
  		</div><?php endforeach; endif; else: echo "" ;endif; ?>
  		<!-- 产品列表结束 -->
