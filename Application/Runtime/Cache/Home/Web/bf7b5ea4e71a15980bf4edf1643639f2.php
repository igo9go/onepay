  ...
  <body>
    ...
    <form method="post" action="<?php echo U();?>" class="form-user">
        ...
        <div class="login-block">
          <h5 class="regtitle">新用户注册<small><a href="<?php echo U('public/login');?>" class="a1">已有账号？立即登录</a></small></h5>
          <div class="input-ls regls user">
            <input type="text" name="username" placeholder="邮箱地址/手机号" />
          ...
          <div class="input-ls regls">
            <input type="text" name="passcode" class="yanzhen" placeholder="验证码" />
            <div class="yanzimg"><img src="<?php echo U('Public/verify');?>" class="passcode" /></div>
            <span class="input-user icon icon-dunpai"></span>
          </div>
          <div class="login-text text-left"><input type="checkbox" name="protocol" checked="checked"><a href="/news/more/id/2" target="_blank">《<?php echo ($web_title); ?>服务协议》</a></div>
          <div class="input-btn"><button type="submit" class="btn btn-big btn-yellow ajax-post" target-form="form-user">同意协议并注册</button></div>
        </div>
		  </div>
    </form>
  	</div>
